...
const roomHeartbeats = new Map<string, NodeJS.Timeout>();

function startHeartbeat(io: Server, roomCode: string): void {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      const syncState = buildSyncState(room.currentTrack);
...
function stopHeartbeat(roomCode: string): void {
...
export function setupPlayerHandlers(io: Server, socket: Socket): void {
  ...
  // Play a track
  socket.on('player:play', async ({ videoId, time, title, thumbnail, channelTitle, duration }: any) => {
    try {
  ...
  // Pause playback
  socket.on('player:pause', async ({ time }: { time: number }) => {
    try {
      const userData = socketUserMap.get(socket.id);
      if (!userData?.roomCode) return;
  ...
  // Seek to time
  socket.on('player:seek', async ({ time }: { time: number }) => {
    try {
      const userData = socketUserMap.get(socket.id);
      if (!userData?.roomCode) return;
  ...
  socket.on('player:ended', async () => {
    try {
      ...
      if (room.hostId.toString() !== userId && !room.settings?.allowGuestControl) return;
